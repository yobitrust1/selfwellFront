...
export default class ChartBuilder {
    ...
    renderXAxisLines() {
        ...
        return [...new Array((this.xAxisLabelCount || 4) + 1)].map((_, i) => {
            return (<Line key={Math.random()} x1={this.leftAlignedXAxisLabelWidth} x2={this.width - (this.leftAlignedXAxisLabelWidth ? 0 : this.xAxisLabelWidth)} y1={this.yDistanceBetweenXLabels * i} y2={this.yDistanceBetweenXLabels * i} stroke={color} strokeDasharray={'5, 10'} strokeWidth={strokeWidth}/>);
        });
    }
    renderYAxisLines() {
        let color = '#00000';
        ...
        return [...new Array(Math.ceil(this.data.length))].map((_, i) => {
            return (<Line key={Math.random()} x1={(i * this.yLabelSlotWidth) + (this.yLabelSlotWidth / 2) + this.leftAlignedXAxisLabelWidth} x2={(i * this.yLabelSlotWidth) + (this.yLabelSlotWidth / 2) + this.leftAlignedXAxisLabelWidth} y1={0} y2={this.baseHeight} stroke={color} strokeDasharray={'5, 10'} strokeWidth={strokeWidth}/>);
        });
    }
    renderXAxisLabels() {
        let xOffset = 0;
        ...
        return [...new Array(this.xAxisLabelCount + 1)].map((_, i) => {
            ...
            return (<Text key={Math.random()} origin={`${x}, ${y}`} x={x} y={y} rotation={rotation} textAnchor='middle' fontFamily={fontFamily} fontSize={fontSize} fontWeight={fontWeight} fill={color}>
          {label}
        </Text>);
        });
    }
