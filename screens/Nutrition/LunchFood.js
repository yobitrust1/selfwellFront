...
var back = '<';
  ...
  const deleteFoodFromLunch = async (food, calories, protein, fiber, saturatedFats, calcium, fat, netCarbs, index) => {
    await API.deleteFoodFromLunch({food, calories, protein, fiber, saturatedFats, calcium, fat, netCarbs }, lunchItems.id, index);
    getfood(pressedDate);
  };

  const showAlertDeleteLunch = (food, calories, protein, fiber, saturatedFats, calcium, fat, netCarbs, index) => {
    Alert.alert(
        'Confirmation',
        ...
        [
            ...
            {
                text: 'Delete',
                onPress: () => deleteFoodFromLunch(food, calories, protein, fiber, saturatedFats, calcium, fat, netCarbs, index)
            },
        ],
    )
  }
    ...
    <Modal
      ...
      <View style={styles.centeredView}>
        ...
        <View style={styles.containerOne}>
          ...
          <TouchableOpacity onPress={() => {navigation.navigate('DailyFood'), setValue('Lunch')}}>
...
{lunchItems &&
           lunchItems.food.map((lunch, index) => (
             <Card1  key={index} style={styles.card}>
               
                    <Block row space="between">
                      <Text style={{color: 'black', fontSize: 16}}>{lunch.name}</Text>
                      <TouchableOpacity 
                                spacing={0.5} caption
                                onPress={() => {showAlertDeleteLunch([lunch], lunch.calories, lunch.protein, lunch.fiber,
                                  lunch.saturatedFats, lunch.calcium, lunch.fat, lunch.netCarbs, index)}}
                      >
                    ...
                    <Block row space="between">
                      ...
                      <Text style={styles.textCalories}>{lunch.calories}</Text>
                    </Block>
            </Card1>
           ))}
    
           <Text style={styles.textCalories}>{"\n"}{parseFloat(lunchItems.calories).toFixed(0)} cals</Text>
           <TouchableOpacity
                     
                     onPress={() => {navigateToLunch()}}
                 >
           ...
           <View style={styles.containerFour}>
             ...
             <View style={{flex: 1, flexDirection: 'column',  marginLeft:width * 0.05}}>
               <Text>{(parseFloat(lunchItems.calories)).toFixed(0)} cals, {(((parseFloat(lunchItems.calories)).toFixed(2)*100)/1800).toFixed(0)}%</Text>
               <ProgressBar progress={(lunchItems.calories)/1800} color={Colors.green300} width={width * 0.5} />
             </View>
             </View>  
             <View style={{flex: 1, flexDirection: 'row', marginTop: height * 0.03 , marginLeft:width * 0.05}}>
           ...
           <View style={{flex: 1, flexDirection: 'column', marginTop: height * 0.05 , marginLeft:width * 0.01}}>
             ...
             <ProgressBar progress={(lunchItems.netCarbs)/212} color={Colors.green500} width={width * 0.25} />
             <Text>{(parseFloat(lunchItems.netCarbs)).toFixed(2)}cals</Text>
           </View>
           <View style={{flex: 1, flexDirection: 'column', marginTop: height * 0.05 , marginLeft:width * 0.01}}>
             <Text>Fat: {(((parseFloat(lunchItems.fat)).toFixed(2)*100)/73).toFixed(0)}%</Text>
             <ProgressBar progress={(lunchItems.fat)/73} color={Colors.green500} width={width * 0.25} />
             <Text>{(parseFloat(lunchItems.fat)).toFixed(2)}cals</Text>
           </View>
           </View>
         ...
         <View style={styles.containerFive}>
           ...
           <View style={{flex: 1, flexDirection: 'row', marginLeft: width * 0.05, marginTop: height * 0.02 }}>
             ...
             <Text style={{color: 'green', fontSize: 15.5, paddingLeft: width * 0.02}}>{(parseFloat(lunchItems.calcium)).toFixed(2)}g</Text>
           </View>
           <View style={{flex: 1, flexDirection: 'row', marginLeft: width * 0.05, marginTop: height * 0.02 }}>
             <Text style={{color: 'black', fontSize: 15.5}}>Fiber: </Text>
             <Text style={{color: 'darkorange', fontSize: 15.5, paddingLeft: width * 0.02}}>{(parseFloat(lunchItems.fiber)).toFixed(2)}g</Text>
           </View>
           <View style={{flex: 1, flexDirection: 'row', marginLeft: width * 0.05, marginTop: height * 0.02 }}>
               <Text style={{color: 'black', fontSize: 15.5}}>Saturated fats: </Text>
               <Text style={{color: 'gold', fontSize: 15.5, paddingLeft: width * 0.02}}>{(parseFloat(lunchItems.saturatedFats)).toFixed(2)}g</Text>
           </View>
           </View>
           </View>):(<ScrollView>
