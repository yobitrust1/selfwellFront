...
var back = '<';
  ...
  const deleteFoodFromDinner = async (food, calories, protein, fiber, saturatedFats, calcium, fat, netCarbs, index) => {
    await API.deleteFoodFromDinner({food, calories, protein, fiber, saturatedFats, calcium, fat, netCarbs }, dinnerItems.id, index);
    getfood(pressedDate);
  };
  const showAlertDeleteDinner = (food, calories, protein, fiber, saturatedFats, calcium, fat, netCarbs, index) => {
    Alert.alert(
        'Confirmation',
        ...
        [
            ...
            {
                text: 'Delete',
                onPress: () => deleteFoodFromDinner(food, calories, protein, fiber, saturatedFats, calcium, fat, netCarbs, index)
            },
        ],
    )
  }
    ...
    <Modal
      ...
      <View style={styles.centeredView}>
        ...
        <View style={styles.containerOne}>
          ...
          <TouchableOpacity onPress={() => {navigation.navigate('DailyFood'), setValue('Dinner')}}>
...
{dinnerItems &&
           dinnerItems.food.map((dinner, index) => (
             <Card1  key={index} style={styles.card}>
               
                    <Block row space="between">
                      <Text style={{color: 'black', fontSize: 16}}>{dinner.name}</Text>
                      <TouchableOpacity 
                                spacing={0.5} caption
                                onPress={() => {showAlertDeleteDinner([dinner], dinner.calories, dinner.protein, dinner.fiber,
                                  dinner.saturatedFats, dinner.calcium, dinner.fat, dinner.netCarbs, index)}}
                      >
                    ...
                    <Block row space="between">
                      ...
                      <Text style={styles.textCalories}>{dinner.calories}</Text>
                    </Block>
            </Card1>
           ))}
    
           <Text style={styles.textCalories}>{"\n"}{parseFloat(dinnerItems.calories).toFixed(0)} cals</Text>
           <TouchableOpacity
                     
                     onPress={() => {navigateToDinner()}}
                 >
           ...
           <View style={styles.containerFour}>
             ...
             <View style={{flex: 1, flexDirection: 'column',  marginLeft:width * 0.05}}>
               <Text>{(parseFloat(dinnerItems.calories)).toFixed(0)} cals, {(((parseFloat(dinnerItems.calories)).toFixed(2)*100)/1800).toFixed(0)}%</Text>
               <ProgressBar progress={(dinnerItems.calories)/1800} color={Colors.green300} width={width * 0.5} />
             </View>
             </View>  
             <View style={{flex: 1, flexDirection: 'row', marginTop: height * 0.03 , marginLeft:width * 0.05}}>
           ...
           <View style={{flex: 1, flexDirection: 'column', marginTop: height * 0.05 , marginLeft:width * 0.01}}>
             ...
             <ProgressBar progress={(dinnerItems.netCarbs)/212} color={Colors.green500} width={width * 0.25} />
             <Text>{(parseFloat(dinnerItems.netCarbs)).toFixed(2)}cals</Text>
           </View>
           <View style={{flex: 1, flexDirection: 'column', marginTop: height * 0.05 , marginLeft:width * 0.01}}>
             <Text>Fat: {(((parseFloat(dinnerItems.fat)).toFixed(2)*100)/73).toFixed(0)}%</Text>
             <ProgressBar progress={(dinnerItems.fat)/73} color={Colors.green500} width={width * 0.25} />
             <Text>{(parseFloat(dinnerItems.fat)).toFixed(2)}cals</Text>
           </View>
           </View>
         ...
         <View style={styles.containerFive}>
           ...
           <View style={{flex: 1, flexDirection: 'row', marginLeft: width * 0.05, marginTop: height * 0.02 }}>
             ...
             <Text style={{color: 'green', fontSize: 15.5, paddingLeft: width * 0.02}}>{(parseFloat(dinnerItems.calcium)).toFixed(2)}g</Text>
           </View>
           <View style={{flex: 1, flexDirection: 'row', marginLeft: width * 0.05, marginTop: height * 0.02 }}>
             <Text style={{color: 'black', fontSize: 15.5}}>Fiber: </Text>
             <Text style={{color: 'darkorange', fontSize: 15.5, paddingLeft: width * 0.02}}>{(parseFloat(dinnerItems.fiber)).toFixed(2)}g</Text>
           </View>
           <View style={{flex: 1, flexDirection: 'row', marginLeft: width * 0.05, marginTop: height * 0.02 }}>
               <Text style={{color: 'black', fontSize: 15.5}}>Saturated fats: </Text>
               <Text style={{color: 'gold', fontSize: 15.5, paddingLeft: width * 0.02}}>{(parseFloat(dinnerItems.saturatedFats)).toFixed(2)}g</Text>
           </View>
           </View>
